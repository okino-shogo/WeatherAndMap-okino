...
struct MyLocationView: View {
    ...
    var body: some View {
        // プレビューでの確認用Text、あとで削除します
        Text("coordinate: \(tappedPoint.latitude), \(tappedPoint.longitude)")//緯度軽度表示用
        MapReader { mapProxy in
            Map(position: $position){//Mapの表示位置をpositionに指定
                UserAnnotation()//ユーザーの位置を表す青ピン
                // マーカー(アノテーション)を配置
            ...
            .onTapGesture { location in
                ...
                if let coordinate = mapProxy.convert(location, from: .local) {
                    ...
                    //myLocationにタップした地点の情報を追加(地図上にピンを追加)
